...
const ReceiverInfo = ({ onChange, resetInput, setResetInput }) => {
    ...
    useEffect(() => {
        const fetchAddressData = async () => {
            try {
                await fetch(
                    "https://raw.githubusercontent.com/daohoangson/dvhcvn/refs/heads/master/data/dvhcvn.json"
                )
    ...
    const filteredWards =
        filteredDistricts.find((district) => district.name === districtName)
            ?.level3s || []
    ...
    const updateReceiverFullAddress = (e) => {
        if (!!wardName && !!districtName && !!cityName) {
            ...
            setReceiverPartAddressLine(`${districtName},${cityName}`)
        }
    }
    useEffect(updateReceiverInfo, [
        cityName,
        districtName,
        wardName,
        phoneNumber,
