...
const CreateRoute = () => {
  ...
  const handleConfirmDeleteRoute = async () => {
    try {
      const response = await api.del(`RoutesControllers/${selectedRouteId}`);
      if (response.success) {
        setAlertMessage("Route deleted successfully!");
        setAlertSeverity("success");
        setRoutes((prevRoutes) =>
          prevRoutes.filter((route) => route.routeId !== selectedRouteId)
        );
    ...
    <Box display="flex">
      ...
      <Box flex={1} padding={3}>
                  ...
                  routes.map((route) => (
                    <React.Fragment key={route.routeId}>
                      <TableRow>
                        <TableCell>
                          <IconButton
                            onClick={() => handleExpandClick(route.routeId)}
                            size="small"
                          >
                        ...
                        <TableCell>
                          <IconButton
                            color="error"
                            onClick={() =>
                              handleDeleteRouteClick(route.routeId)
                            }
                          >
                            <DeleteIcon />
                          </IconButton>
                      ...
                      <TableRow>
                        ...
                        >
                          <Collapse
                            in={expandedRouteId === route.routeId}
                            timeout="auto"
                            unmountOnExit
                          >
                            <Box margin={2}>
                              <Typography variant="h6" gutterBottom>
                                Orders for Route {route.routeId}
                              </Typography>
                              <Table size="small" aria-label="orders">
                                ...
                                <TableBody>
                                  {orders[route.routeId] &&
                                  orders[route.routeId].length > 0 ? (
                                    orders[route.routeId].map((order) => (
        ...
        >
          ...
          >
            <Typography variant="h6" id="delete-order-modal-title" gutterBottom>
              Confirm Delete
            </Typography>
            <Typography id="delete-order-modal-description" sx={{ mb: 2 }}>
        ...
        >
          ...
          >
            <Typography variant="h6" id="delete-route-modal-title" gutterBottom>
              Confirm Delete Route
            </Typography>
            <Typography id="delete-route-modal-description" sx={{ mb: 2 }}>
