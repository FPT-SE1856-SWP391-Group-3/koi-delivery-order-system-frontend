...
export default function ManageOrderServiceDetail() {
    ...
    async function deleteOrderService(orderServiceDetailId) {
        try {
            console.log(url('OrderServices/deleteOrderServiceDetail/' + orderServiceDetailId));
            const response = await fetch(url('OrderServices/deleteOrderServiceDetail/' + orderServiceDetailId), {
                method: 'DELETE',
                headers: {
            ...
            if (data.success) {
                ...
                const newOrderServiceDetails = orderServiceDetails.filter(orderServiceDetail => orderServiceDetail.orderServiceDetailId !== orderServiceDetailId);
        ...
        <div>
            ...
            {orderServiceDetails.map((orderServiceDetail) => (
                <div key={orderServiceDetail.orderServiceDetailId}>
                    ...
                    <button onClick={() => deleteOrderService(orderServiceDetail.orderServiceDetailId)}>Delete</button>
                    <a href={"/admin/updateOrderServiceDetail/" + orderServiceDetail.orderServiceDetailId}>Update</a>
                </div>
            ))}
        </div>
    );
